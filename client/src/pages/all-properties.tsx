    ...
    <Box>
          ...
          <Box mb={2} mt={3} display='flex' width='84%' justifyContent='space-between' flexWrap='wrap'>
            <Box display='flex' gap={2} flexWrap='wrap' mb={{ xs: '20px', sm: 0 }}>
              ...
              >
                ...
                {['Appartment', 'Villa', 'Farmhouse', 'Condos', 'Townhouse', 'Duplex', 'Studio', 'Chalet'].map(type => (<MenuItem key={type} value={type.toLocaleLowerCase()}>{type}</MenuItem>))}
              </Select>
            </Box>
          </Box>
        </Stack>
      ...
      {allProperties.length > 0 && (
        <Box display='flex'gap={2} mt={3} flexWrap='wrap'>
          ...
          >
            {[10, 20, 30, 40, 50].map((size) => (<MenuItem key={size} value={size}>{size}</MenuItem>))}
          </Select>
        </Box>
      )}
    </Box>
